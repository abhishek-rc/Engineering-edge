...
const QuotesTableContainer: FunctionComponent = () => {
  ...
  const QuotesTablePageHeader = () => {
    return (
      <PageHeader
        title={<FormattedMessage id="admin/b2b-quotes.quotes-table.title" />}
        linkLabel={<FormattedMessage id="admin/b2b-quotes.back" />}
        onLinkClick={() =>
          history.goBack()
          // navigate({
          //   to: `${rootPath ?? ''}/account`,
  ...
  if (
    ...
    const message = isAuthenticated ? (
      <FormattedMessage id="admin/b2b-quotes.error.notPermitted" />
    ) : (
      <FormattedMessage id="admin/b2b-quotes.error.notAuthenticated" />
    )
