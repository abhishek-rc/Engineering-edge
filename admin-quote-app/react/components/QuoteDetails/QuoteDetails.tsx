...
const QuoteDetails: FunctionComponent = () => {
  ...
  useEffect(() => {
    if (sellersData?.sellers?.items) {
      const sellerOptions = sellersData.sellers.items.map((seller: any) => ({
        value: seller.id,
        label: seller.name
      }))
  ...
  const handleSellerChange = (value: any) => {
    ...
    // Find the selected seller object to log its name
    if (value === null) {
      return
    }
  ...
  // const handlePercentageDiscount = (newSubtotal: number) => {
  //   // Calculate the discount amount based on the difference between original and new subtotal
  //   const discountAmount = originalSubtotal - newSubtotal
  //   // Calculate the percentage discount
  //   const percent = originalSubtotal > 0 ? (discountAmount / originalSubtotal) * 100 : 0
  //   setDiscountState(percent)

  //   let newItems = [] as QuoteItem[]

  //   if (newSubtotal >= originalSubtotal) {
  //     // If no discount or negative discount (which shouldn't happen), reset to original prices
  //     if (isNewQuote) {
  ...
  //     const discountRatio = newSubtotal / originalSubtotal

  //     // Apply the discount ratio to each item's price
  //     quoteState.items.forEach((item: QuoteItem) => {
  //       const newSellingPrice = Math.round(item.price * discountRatio)
                    ...
                    <div className="nowrap">
                      ...
                      {quoteDeclinable && (
                        <span className="mr4">
                          ...
                          >
                            <FormattedMessage id="admin/b2b-quotes.quote-details.decline" />
                          </Button>
                        </span>
                      )}
                      ...
                      {quoteUsable && (
                        <span className="mr4">
                          ...
                          >
                            <FormattedMessage id="admin/b2b-quotes.quote-details.use-quote" />
                          </Button>
                        </span>
                      )}
                    ...
                    {/* {formState.isEditable && isSalesRep && (
                      <div className="mt3">
                        <h3 className="t-heading-4 mb4">
                          <FormattedMessage id="admin/b2b-quotes.quote-details.apply-discount.title" />
                        </h3>
                        <div className="pa5">
                          <PercentageDiscount
                            updatingSubtotal={updatingSubtotal}
                          ...
                          {maxDiscountState < 100 && (
                            <div className="mt1">
                              <FormattedMessage
                                id="admin/b2b-quotes.quote-details.apply-discount.maxDiscount-text"
                      ...
                      isSalesRep && (
                        <div className="mt3">
                          <h3 className="t-heading-4">
                            <FormattedMessage id="admin/b2b-quotes.quote-details.expiration-date-change.title" />
                          </h3>
                          <div className="pa5">
                            <DatePicker
                      ...
                      <div className="mt3 pa5">
                        <Textarea
                          ...
                          characterCountdownText={
                            <FormattedMessage
                              id="admin/b2b-quotes.create.characterLeft"
