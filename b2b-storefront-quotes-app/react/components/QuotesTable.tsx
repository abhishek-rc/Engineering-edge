    ...
    properties: {
      ...
      referenceName: {
        ...
        cellRenderer: ({
          ...
          // Make text bold if assignedTo is 'customer' (case-insensitive)
          if (assignedTo && assignedTo.toLowerCase() === 'customer') {
            renderedName = <strong>{rowLoading ? <Spinner /> : referenceName}</strong>
          } else if (
            (isSalesRep && !viewedBySales) ||
            (!isSalesRep && !viewedByCustomer)
          ) {
            renderedName = <span>{referenceName}</span>
          }
